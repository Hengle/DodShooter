...
void UDodDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams,
                                                 FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	FDodGameplayEffectContext* TypedContext = FDodGameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	check(TypedContext);
	...
	if (HitActor)
	{
		UDodTeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UDodTeamSubsystem>();
	...
	if (DamageDone > 0.0f)
	{
		...
		OutExecutionOutput.AddOutputModifier(
			FGameplayModifierEvaluatedData(UDodHealthSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}

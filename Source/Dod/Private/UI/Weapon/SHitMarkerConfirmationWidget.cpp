#include "UI/Weapon/SHitMarkerConfirmationWidget.h"
                                            ...
                                            const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	if (bDrawMarkers)
	{
		...
		{
			if (UDodWeaponStateComponent* WeaponStateComponent = PC->FindComponentByClass<UDodWeaponStateComponent>())
			{
		...
		if ((LastWeaponDamageScreenLocations.Num() > 0) && (PerHitMarkerImage != nullptr))
		{
			...
			for (const FDodScreenSpaceHitLocation& Hit : LastWeaponDamageScreenLocations)
			{
                                        ...
                                        const float InDeltaTime)
{
	...
	if (APlayerController* PC = MyContext.IsInitialized() ? MyContext.GetPlayerController() : nullptr)
	{
		if (UDodWeaponStateComponent* DamageMarkerComponent = PC->FindComponentByClass<UDodWeaponStateComponent>())
		{
